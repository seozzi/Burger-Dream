...
public class FinaleManager : MonoBehaviour
{
    ...
    private void SpawnSlidesAndFood()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            if (slideCollider == null) slideCollider = slide.AddComponent<MeshCollider>();
            ...
            if (foodPrefabs != null && foodPrefabs.Length > 0)
            {
                ...
                {
                    Vector3 foodSpawnPos = slideSpawnPos + Vector3.up * 16.0f - (directionFromCenter * 5.0f);
                    ...
                    {
                        Vector3 textSpawnPos = foodSpawnPos + (Vector3.up * 2f) - (directionFromCenter * 2.0f);
